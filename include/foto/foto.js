$.fn.fotoUpload = function (obj) {
    ...
    $("#foto_upload_" + id).click(function () {
        ...
        html += "<FORM method=post action='/include/foto/fotoUploadFile.php?" + G.values + "' enctype='multipart/form-data' target=dialog_upload_frame id=dialog_form>";
        ...
        function fotoLinkSend() {
            ...
            if (link && link != direct_value) {
                ...
                $.post("/include/foto/fotoUploadLink.php?" + G.values, {link:link}, function (res) {
    ...
    function uploaded(link, x, y) {
        ...
        send.img = "<IMG src='" + send.link + "s.jpg'>";
        obj.func(send);
    }
    ...
    function camera() {
        ...
        function submit() {
            ...
            var timer = setInterval(uploadStart, 500);
            document.getElementById('webcam_movie')._snap("/include/foto/fotoUploadWebcam.php?" + G.values, 100, 0, 0);
            function uploadStart() {
                var cookie = getCookie('fotoUpload');
                if (cookie != 'process') {
                    clearInterval(timer);
...
$.fn.fotoSet = function (obj) {
    ...
    $(this).html("<IMG src='" + foto.link + "b.jpg' width=" + x +" height=" + y + " val='"+ val +"'>");
...
// просмотр фотографий
G.fotoView = function (str, num) {
    var spisok = str.split('_');
    var scroll = G.vkScroll;
    var firstOpen = 1; // Посмотр первой фотографии. Для того, чтобы сохранять размер в процессе загрузки следующей фото.
    num = num || 0; // текущий номер в массиве выводимого изображения

    $("#foto_view").remove();
    $("#frameBody").append("<DIV id=foto_view></DIV>");

    var html ="<DIV id=foto_content val=end_>" +
        "<DIV id=foto_head><EM></EM><A val=fotoClose_>Закрыть</A></DIV>" +
        "<DIV id=foto_image val=" + (spisok.length > 1 ? "fotoNext_" : "fotoClose_") + "></DIV>" +
        "<DIV id=foto_hide></DIV>" +
        "<DIV id=foto_about></DIV>" +
        "</DIV>";
    ...
    function fotoPrint() {
        ...
        if (firstOpen == 1) heightSet();
        firstOpen = 0;
        num++;
        if (num == len) { num = 0; }
        if (len > 1) { // если фотографий больше одной, прячется следующая для предзагрузки
            sp = spisok[num];
            $("#foto_hide").html("<IMG src='" + sp.link + "b.jpg'>");
        }
    }

    // Установка высоты окна контакта после вывода изображения
    function heightSet() {
        var h = foto.height();
        $("#frameBody").height(h);
