...
submit:a});$("#name").focus().keyEnter(a);$("#show_worker")._check()}).on("click","#setup_expense .img_edit",function(){function a(){var a={op:"setup_expense_edit",id:c,name:$("#name").val(),show_worker:$("#show_worker").val()};a.name?(e.process(),$.post(AJAX_GAZ,a,function(a){a.success?($("#spisok").html(a.html),e.close(),_msg("Сохранено!"),sortable()):e.abort()},"json")):(e.bottom.vkHint({msg:"<SPAN class=red>Не указано наименование</SPAN>",top:-47,left:99,indent:50,show:1,remove:1}),$("#name").focus())}
