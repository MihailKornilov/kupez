...
if(count($spisok)>0)
	...
	if($_GET['user']>0)
		...
		$send[0]->vk_name=utf8($vkUs->first_name." ".$vkUs->last_name);
		$send[0]->photo=$vkUs->photo;
		$send[0]->ob_count=utf8($vkUs->ob_count." ���������".obEnd($vkUs->ob_count));
		}
	foreach($spisok as $n=>$sp)
		{
		$send[$n]->id=$sp->id;
		$send[$n]->txt=utf8($sp->txt);
		$send[$n]->rub=utf8($rubrika[$sp->rubrika]);
		$send[$n]->podrub=utf8($podrubrika[$sp->podrubrika]);
		$send[$n]->telefon=utf8($sp->telefon);
		$send[$n]->adres=utf8($sp->adres);
		$send[$n]->file=$sp->file;
		$send[$n]->dtime=utf8(FullDataTime($sp->dtime_add));
		$send[$n]->viewer_id=$sp->viewer_id_add;
		$send[$n]->viewer_id_show=$sp->vk_viewer_id_show;
		if($_GET['user']==0) $vkUs=$VK->QueryObjectOne("select first_name,last_name from vk_user where viewer_id=".$sp->viewer_id_add." limit 1");
		$send[$n]->vk_name=utf8($vkUs->first_name." ".$vkUs->last_name);
//		$send[$n]->dop=$sp->dop;
		}
	if(count($spisok)==$CP)
		{
