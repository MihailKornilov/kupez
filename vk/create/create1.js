...
// список городов
for (var n = 0; n <= 1; n++) { create.cities[n].content = "<B>" + create.cities[n].title + "</B>"; }
if (G.vk.city > 0) {
  VK.api('places.getCityById', {cids:G.vk.city}, function (res) {
    create.cities.unshift({
...
function cityPrint() {
  ...
  $("#vkSel_cities").bind({
    mouseenter:function () {
      $("#ms_adres").alertShow({
        width:205,
        txt:"При указании города Ваше объявление будет отображаться в поиске, если пользователь укажет этот город, иначе только в общем списке.",
        ugol:'left',
        left:178,
        top:-24,
        otstup:30,
...
$("#butts .vkCancel BUTTON").click(function () { location.href = "/index.php?" + G.values + "&my_page=" + create.back; });
...
function vkCreateGo() {
  ...
  if (MSG) {
    ...
    if (create.order.votes > 0 && create.order.id == 0) {
      ...
      $.post("/vk/create/AjaxObCreate.php?" + G.values, obj, function (res) {
//        alert(res)
        location.href = "/index.php?" + G.values + "&my_page=vk-ob";
      },'json');
    }
  }
}
