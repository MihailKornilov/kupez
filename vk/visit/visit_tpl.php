<?php
if(isset($WR[$vkUser->viewer_id])) Header("Location:".$URL); // переводим на главную страницу, если пользователю нельзя смотреть эту
include('incHeader.php');

/*
// УСТАНОВКА ВРЕМЕНИ ПОСЛЕДНЕГО ВХОДА ВСЕМ ПОСЕТИТЕЛЯМ
...
// УСТАНОВКА КОЛИЧЕСТВА РАЗМЕЩЁННЫХ ОБЪЯВЛЕНИЙ ВСЕМ ПОСЕТИТЕЛЯМ
$spisok=$VK->QueryObjectArray("select * from vk_user order by viewer_id");
foreach($spisok as $sp)
	{
...
// УСТАНОВКА ВСЕМ - УСТАНОВИЛ ЛИ ПОЛЬЗОВАТЕЛЬ ПРИЛОЖЕНИЕ
...
// УСТАНОВКА ВСЕМ - УСТАНОВИЛ ЛИ ПОЛЬЗОВАТЕЛЬ ССЫЛКУ В ЛЕВОЕ МЕНЮ
//$VK->Query("update vk_user set app_setup=0");
require_once('include/vkapi.class.php');
$VKAPI = new vkapi(2881875,'h9IjOkxIMwoW8agQkW3M');
$spisok=$VK->QueryObjectArray("select * from vk_user order by viewer_id desc limit 500,100");
