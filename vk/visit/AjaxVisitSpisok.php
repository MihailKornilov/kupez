...
$send[0]->result=iconv("WINDOWS-1251","UTF-8",$fCount.$send[0]->count." посетител".visitEnd($send[0]->count));
$send[0]->page=0;
...
if(count($spisok)>0)
	...
	foreach($spisok as $n=>$sp)
		...
		$send[$n]->first_name=iconv("WINDOWS-1251","UTF-8",$sp->first_name);
		$send[$n]->last_name=iconv("WINDOWS-1251","UTF-8",$sp->last_name);
		...
		else $send[$n]->time=iconv("WINDOWS-1251","UTF-8",FullDataTime($sp->enter_last,1));
