...
switch(@$_POST['op']) {
	...
	case 'ob_create':
		...
		$send['msg'] = utf8(htmlspecialchars_decode(
			_rubric($rubric_id).
			($rubric_sub_id ? ' ї '._rubricsub($rubric_sub_id) : '').': '.
			trim(substr($txt, 0, 100))."...".
			($telefon ? "\n".'&#9742; '.$telefon : '').//&#128222;
			//($viewer_id_show ? ' @id'.VIEWER_ID.'('._viewer(VIEWER_ID, 'name').')' : '').
			"\n".'„итайте полностью на vk.com/kupezz'
		));
		...
		if(mysql_num_rows($q)) {
			...
			while($r = mysql_fetch_assoc($q)) {
				...
				if(!$n) {
					...
					$image_post_url = $r['path'].$big_name; //изображение дл€ сохранени€ на стену
				}
				$n++;
			}
			query("UPDATE `vk_ob` SET `image_id`=".$image_id.",`image_link`='".$image_link."' WHERE `id`=".$insert_id);

			$group_id = 72078602;   //√руппа  упецЏ
			$album_id = 195528889;  //ќсновной альбом
