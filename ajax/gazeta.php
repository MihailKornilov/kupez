...
switch(@$_POST['op']) {
		...
		if($dop != $p['dop']) {
			...
			_historyInsert(
				...
				array(
					...
					'value1' => '<table>'.
								'<td>'._polosa($p['dop']).($p['polosa'] ? ' '.$p['polosa'].'-я' : '').
								'<td>»'.
								'<td>'._polosa($dop).
							   '</table>'
				),
			...
			_historyInsert(
				...
				array(
					...
					'value1' => '<table>'.
								'<td>'._polosa($p['dop']).($p['polosa'] ? ' '.$p['polosa'].'-я' : '').
								'<td>»'.
								'<td>'._polosa($p['dop']).($polosa ? ' '.$polosa.'-я' : '').
							   '</table>'
				),
				'gazeta_history'
			);
	...
	case 'zayav_edit':
		...
		//Изменение объявления в общем доступе
		if($z['category'] == 1) {
			...
			if($r = query_assoc($sql)) {
				$sql = "UPDATE `vk_ob`
						...
						WHERE `id`=".$r['id'];
				query($sql);
				//Обновление даты выхода объявления
				$sql = "SELECT IFNULL(MAX(`general_nomer`),0)
						FROM `gazeta_nomer_pub`
						WHERE `zayav_id`=".$zayav_id;
				if($max = query_value($sql)) {
	...
	case 'report_zayav_nomer':
		if(!$nomer = _isnum($_POST['nomer']))
			jsonError();
		...
		if($year) {
			$send['gn_sel'] = gnJson($year, 1);
			$sql = "SELECT `general_nomer` FROM `gazeta_nomer` WHERE `day_public` LIKE '".$year."%' ORDER BY `day_public` LIMIT 1";
			$nomer = query_value($sql);
			$send['gn_first'] = $nomer;
		}

		$send['spisok'] = utf8(report_zayav_nomer($nomer));
